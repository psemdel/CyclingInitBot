...
def proTeamDescription(teamTable, kk, Year):
    ...
    mydescription[u'fr'] = description_part1_fr + " " + \
        str(Year) + " " + description_part2_fr + " " + name
...
def proteamcreator(
    ...
    if 1 == 1:
        for kk in range(kkinit, endkk):  # endkk
            if (proamateur == 1 and teamTableFemmes[kk][6] == 1) or (
                    proamateur == 0 and teamTableFemmes[kk][5] == 1):
                ...
                proTeamBasic(
                    ...
                    Year,
                    teamTableFemmes[kk][2],
                    teamTableFemmes[kk][4],
                    proamateur)
                time.sleep(1.0)
                ...
                # Search previous
                Yearprevious = Year - 1
                mylabelprevious = proTeamLabel(
                    teamTableFemmes, kk, Yearprevious)
                ...
                itemMaster = pywikibot.ItemPage(
                    repo, u'Q' + str(teamTableFemmes[kk][2]))
                itemMaster.get()
                addMultipleValue(
                    pywikibot,
                    repo,
