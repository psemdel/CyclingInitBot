...
def dateSorterEmpty(pywikibot,site,repo,time,IdTeamPage,VicOrComp, listOfItem):
...
def nameSorter(pywikibot,site,repo,time,IdTeamPage, TeamOrOther):
   ...
   # print(listeDesCyclistes[0].fullName)     
    ...
    nameToParse=[[u'' for x in range(3)] for y in range(len(nameToParseInput))]
    ...
    for ii in range(len(sortedName)): #
        ...
        deleteValue(pywikibot,repo,item,propertyNumber,noQ(itemID),'rider for sorting')
        addMultipleValue(pywikibot,repo,item,propertyNumber, noQ(itemID),'rider for sorting',1)
