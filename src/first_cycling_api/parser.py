...
def parse_startlist(tables, res):
    """ Convert HTML table from bs4 to pandas DataFrame. Return None if no data. """
    ...
    try:
        ...
        bib_df = bib_df.set_index(bib_df["Inv name"])

        for i in res.index:
            try:
                res.loc[i,"BIB"]=bib_df.loc[res.loc[i,"Inv name"]]["BIB"]
            except:
                print(res.loc[i,"Inv name"] + " not found")
                res.loc[i,"BIB"]=0
