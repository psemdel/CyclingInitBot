    ...
    for ii in range( startYear,EndYear):
        ...
        if(u'P527' in item.claims):
             ...
             for ii in range(len(listOfcomprend)):#   
                 ...
                 Idchamp=searchItem(pywikibot,site,thislabel[thislabel.find('championnats'):])
                 if (Idpresent==u'Q0'or Idpresent==u'Q1'):
                     print(thislabel+ "not found")
                 else:
                     addValue(pywikibot,repo,itemthisChampionship,361,noQ(Idchamp),u'link master') 
    ...
    for ii in range( startYear,EndYear):
        ...
        if (Idprevious==u'Q0')or(Idprevious==u'Q1'):  #no previous or several
           a=1
            ...
            addValue(pywikibot,repo,itemson,155,noQ(Idprevious),u'link previous') 
        ...
        if(u'P527' in item.claims):
             ...
             for ii in range(len(listOfcomprend)): 
                 ...
                 if(u'P527' in itemthisChampionship.claims):
                     listOfcomprendchamp=itemthisChampionship.claims.get(u'P527')
                     for jj in range(len(listOfcomprendchamp)):
                          ...
                          if racelabel.find(prefix)==0:
                              itemthisraceID =listOfcomprendchamp[jj].getTarget().getID()
                              addMultipleValue(pywikibot,repo,itemson,527, noQ(itemthisraceID),u'duplicate',0) 
