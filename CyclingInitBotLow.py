...
def isitacyclist(pywikibot, repo, itemId):
    ...
    if(u'P106' in item.claims):  # already there do nothing
        ...
        for ii in range(len(listOfoccupation)):
            if listOfoccupation[ii].getTarget(
            ) == cyclistoccupation:  # Already there
                return 1
    return 0
    ...
    if (u'P54' in item.claims):
        ...
        for mm in range(len(allteams)):
            ...
            if ('P582' in allteams[mm].qualifiers):
                ...
                endtime = pywikibot.WbTime(
                    site=site, year=2100, month=1, day=1, precision='day')
            if (compareDates(begintime,
                             timeOfRace) == 2 or compareDates(begintime,
                                                              timeOfRace) == 0) and (compareDates(endtime,
                                                                                                  timeOfRace) == 1 or compareDates(begintime,
                                                                                                                                   timeOfRace) == 0):
