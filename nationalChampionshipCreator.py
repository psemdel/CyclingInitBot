...
def nationalChampionshipEnLigneBasic(
        pywikibot,
        repo,
        item,
        siteIn,
...
def continentalChampionshipEnLigneBasic(
        pywikibot, repo, item, siteIn, Master, Year):
    addValue(pywikibot, repo, item, 31, Master, u'Nature')
    Addc = 1
...
def NationalChampionshipCreator(
        pywikibot,
        site,
        repo,
        time,
    ...
    for kk in range(kkinit, endkk):  # endkk
        ...
        if group == 1:
            for ii in range(startYear, EndYear):
                ...
                # Search previous
                Yearprevious = Year - 1
                mylabelprevious = nationalChampionshipLabel(
                    teamTable, kk, Yearprevious)
                ...
                # Search next
                Yearnext = Year + 1
                mylabelnext = nationalChampionshipLabel(
                    teamTable, kk, Yearnext)
                ...
                if teamTable[kk][IndexRoadRace] != 0:
                    time.sleep(1.0)
                    mylabelEnLigne = nationalChampionshipEnLigneLabel(
                        teamTable, kk, Year, ManOrWoman)
                    IdEnLignepresent = searchItem(
                        pywikibot, site, mylabelEnLigne['fr'])
                    if (IdEnLignepresent == u'Q0'):
                        print(mylabelEnLigne['fr'] + ' created')
                        # Type code
                        IdEnLignepresent = create_item(
                            pywikibot, site, mylabelEnLigne)

                    elif (IdEnLignepresent == u'Q1'):
                        print(
                            mylabelEnLigne['fr'] +
                            ' already present several times')
                    else:
                    ...
                    if CC == u'no':
                        nationalChampionshipEnLigneBasic(
                            pywikibot,
                            repo,
                            itemEnLigne,
                            site,
                            teamTable[kk][IndexRoadRace],
                            teamTable[kk][3],
                            Year,
                            IdEnLignepresent,
                            1)
                    else:
                        continentalChampionshipEnLigneBasic(
                            pywikibot, repo, item, siteIn, teamTable[kk][IndexRoadRace], Year)

                    # Link to previous
                    mylabelEnLigneprevious = nationalChampionshipEnLigneLabel(
                        teamTable, kk, Yearprevious, ManOrWoman)

                    IdEnLigneprevious = searchItem(
                        pywikibot, site, mylabelEnLigneprevious['fr'])
                    if (IdEnLigneprevious == u'Q0')or(
                            IdEnLigneprevious == u'Q1'):  # no previous or several
                        a = 1
                    else:
                        addValue(
                            pywikibot,
                            ...
                            itemEnLigne,
                            155,
                            noQ(IdEnLigneprevious),
                            u'link previous')
                        # Link to the previous
                        itemEnLignePrevious = pywikibot.ItemPage(
                            repo, IdEnLigneprevious)
                        ...
                        addValue(
                            ...
                            itemEnLignePrevious,
                            156,
                            noQ(IdEnLignepresent),
                            u'link next')

                    # Link to next
                    mylabelEnLignenext = nationalChampionshipEnLigneLabel(
                        teamTable, kk, Yearnext, ManOrWoman)
                    IdEnLignenext = searchItem(
                        pywikibot, site, mylabelEnLignenext['fr'])

                    time.sleep(1.0)
                    if (IdEnLignenext == u'Q0')or(
                            IdEnLignenext == u'Q1'):  # no next or
                        a = 1
                    else:
                        addValue(
                            pywikibot,
                            ...
                            u'link next')
                        # Link to the next
                        itemEnLigneNext = pywikibot.ItemPage(
                            repo, IdEnLignenext)
                        itemEnLigneNext.get()
                        addValue(
                            pywikibot,
                    ...
                    itemEnLigneMaster = pywikibot.ItemPage(
                        repo, u'Q' + str(teamTable[kk][IndexRoadRace]))
                    ...
                    addMultipleValue(pywikibot, repo, item, 527, noQ(
                        IdEnLignepresent), u'link course en ligne', 0)

                 # Create the Clm
                time.sleep(1.0)
                if clm == 1 and teamTable[kk][IndexClmRace] != 0:
                    mylabelClm = nationalChampionshipClmLabel(
                        teamTable, kk, Year, ManOrWoman)
                    IdClmpresent = searchItem(
                        pywikibot, site, mylabelClm['fr'])
                    ...
                    # Link to previous
                    mylabelClmprevious = nationalChampionshipClmLabel(
                        teamTable, kk, Yearprevious, ManOrWoman)
                    IdClmprevious = searchItem(
                        pywikibot, site, mylabelClmprevious['fr'])
                    if (IdClmprevious == u'Q0')or(
                            IdClmprevious == u'Q1'):  # no previous or several
                        a = 1
                    else:
                        addValue(
                            pywikibot,
                            ...
                            u'link previous')
                        # Link to the previous
                        itemClmPrevious = pywikibot.ItemPage(
                            repo, IdClmprevious)
                        itemClmPrevious.get()
                        addValue(
                            pywikibot,
                    ...
                    # Link to next
                    mylabelClmnext = nationalChampionshipClmLabel(
                        teamTable, kk, Yearnext, ManOrWoman)
                    IdClmnext = searchItem(
                        pywikibot, site, mylabelClmnext['fr'])

                    time.sleep(1.0)
                    if (IdClmnext == u'Q0')or(
                            IdClmnext == u'Q1'):  # no next or
                        a = 1
                    else:
                        addValue(
                            pywikibot,
                    ...
                    # Link to master (clm...)
                    itemClmMaster = pywikibot.ItemPage(
                        repo, u'Q' + str(teamTable[kk][IndexClmRace]))
                    itemClmMaster.get()
                    addMultipleValue(
                        pywikibot,
                        repo,
